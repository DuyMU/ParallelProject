...
int main(int argc, const char *argv[]){
	...
	param_t *package = (param_t *)malloc(num_threads * sizeof(param_t));
        for(countThread = 0; countThread < num_threads; countThread++){
		int *rowsTransfer = malloc(sizeof(int));
		*rowsTransfer = position + num_rows_per_thread[countThread]; 
		
		package[countThread].id = position;
		package[countThread].rows = rowsTransfer;
		package[countThread].dim = dim;
		package[countThread].m_a = matrix_a;
		package[countThread].m_b = matrix_b;
		package[countThread].m_r = result;

		//pthread_create(&thread[countThread], &attr, worker, (void *) package);

		position += num_rows_per_thread[countThread];
	}

	for(i = 0; i < num_threads; i++){
            pthread_create(&(thread[i]),&attr,worker, (void *) &(package[i]));
        }
