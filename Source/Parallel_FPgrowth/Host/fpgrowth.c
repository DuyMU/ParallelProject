...
static ItemLink *transaction = NULL;             /* array of transactions */
...
int main(int argc, char *argv[])
{
  ...
  if(inFile && *inFile){
	...
	transaction = (ItemLink *)calloc(transNum, sizeof(ItemLink));
	loadTransactionFile(in, transaction, hashTable, (int)support, transNum, itemsNum);
	deleteUnsupport(transaction, transNum, (int)support, hashTable, itemsNum);
	sortEveryTransaction(transaction, transNum, hashTable, itemsNum, (int)support);
	...
	createFpTree(fptree, headTable, transaction, transNum);
	...
	fclose(in);
	fclose(out);
	freeTransaction(transaction, transNum);
	freeHashTable(hashTable, itemsNum);
	return 0;
